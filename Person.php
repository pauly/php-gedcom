...
class Person {
  ...
  function name ($link = false, $years = false, $schema = false) {
  ...
  function _place ($type = 'BIRT', $itemProp = false, $default = null) {
  ...
  function td ($person, $schemaRelationship, $columns, $class = null) {
    $html = '<td colspan="' . $columns . '"';
    if ($class) {
      $html .= ' class="' . $class . '"';
    }
    if (!$person->id() || $person->isPrivate()) $schemaRelationship = null;
  ...
  function tableTree ($schema = false) {
    ...
    if ($schema) $html .= ' itemscope itemtype="http://schema.org/Person" id="' . $this->link() . '"';
    $html .= '>';
  ...
  function _relationship($person) {
    ...
    foreach ($person->spouses() as $spouse) {
      if ($this->id() == $spouse->id()) return self::i18n('married');
    }
